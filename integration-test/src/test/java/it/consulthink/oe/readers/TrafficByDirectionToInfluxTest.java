...
public class TrafficByDirectionToInfluxTest {
	private static final Logger LOG = LoggerFactory.getLogger(TrafficByDirectionToInfluxTest.class);
	...
	public void testGetSink1() throws Exception {
		...
		List<Tuple2<Date, Traffic>> iterable = Arrays.asList(Tuple2.of(new Date(), new Traffic(1l,1l,1l)), Tuple2.of(new Date(), new Traffic(2l,2l,2l)),
				Tuple2.of(new Date(), new Traffic(3l,3l,3l)), Tuple2.of(new Date(), new Traffic(4l,4l,4l)));
		...
		DataStreamSink<Tuple2<Date, Traffic>> addSink = senv.fromCollection(iterable).keyBy(0).addSink(sink);
	...
	public void testGetSink1Infinite() throws Exception {
		...
		List<Tuple2<Date, Traffic>> iterable = Lists.newArrayList(Stream.generate(new Supplier<Tuple2<Date, Traffic>>() {

			@Override
			public Tuple2<Date, Traffic> get() {

				try {
					Thread.sleep(500);
				} catch (Throwable e) {}

				return Tuple2.of(new Date(), new Traffic((long) r.nextInt(1024), (long) r.nextInt(1024), (long) r.nextInt(1024)));
			}
...
//		FromIteratorFunction<Tuple2<Date, Traffic>> source = new FromIteratorFunction<Tuple2<Date, Long>>(iterator);
		DataStreamSink<Tuple2<Date, Traffic>> sinked = senv.fromCollection(iterable).keyBy(0).addSink(sink);
