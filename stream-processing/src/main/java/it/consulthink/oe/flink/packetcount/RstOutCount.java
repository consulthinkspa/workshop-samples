 ...
 */
public class RstOutCount extends AbstractApp {

    // Logger initialization
    private static final Logger LOG = LoggerFactory.getLogger(RstOutCount.class);
    ...
    public RstOutCount(AppConfiguration appConfiguration){
        super(appConfiguration);
    }

    public void run(){

        LOG.info("Starting NMA RstOutCount...");
        ...
        dataStream.addSink(sink).name("NMARstOutCountStream");
        ...
        try {
            env.execute("RstOutCount");
        } catch (Exception e) {
            LOG.error("Error executing RstOutCount...");
        }finally {
            LOG.info("Ending NMA RstOutCount...");
        }
    ...
    public static ProcessAllWindowFunction<NMAJSONData, Long, TimeWindow> getProcessAllWindowFunction() {
        ProcessAllWindowFunction<NMAJSONData, Long, TimeWindow> sumRstOut = new ProcessAllWindowFunction<NMAJSONData, Long, TimeWindow>() {
            ...
            public void process(ProcessAllWindowFunction<NMAJSONData, Long, TimeWindow>.Context ctx,
                ...
                for (NMAJSONData element : iterable) {
                    collector.collect(element.getRstout());
                }
            }
    ...
    public static ProcessWindowFunction<NMAJSONData, Tuple2<Date, Long>, Date, TimeWindow> getProcessFunction() {
        ProcessWindowFunction<NMAJSONData, Tuple2<Date, Long>, Date, TimeWindow> sumRstOut = new ProcessWindowFunction<NMAJSONData, Tuple2<Date, Long>, Date, TimeWindow>() {
            ...
            public void process(Date key,ProcessWindowFunction<NMAJSONData, Tuple2<Date, Long>, Date, TimeWindow>.Context ctx,Iterable<NMAJSONData> iterable, Collector<Tuple2<Date, Long>> collector) throws Exception {
                for (NMAJSONData element : iterable) {
                    collector.collect(Tuple2.of(key, element.getRstout()));
                }
            }
        };
        return sumRstOut;
    ...
    private FlinkPravegaWriter<Tuple2<Date, Long>> getSinkFunction(PravegaConfig pravegaConfig, String outputStreamName) {
        ...
        FlinkPravegaWriter<Tuple2<Date, Long>> sink = FlinkPravegaWriter.<Tuple2<Date, Long>>builder()
                ...
                .withEventRouter((a) -> "RstOutPerSecond")
//                .withSerializationSchema(?)
    ...
    public static void main(String[] args) throws Exception {
        LOG.info("Starting RstOutCount...");
        AppConfiguration appConfiguration = new AppConfiguration(args);
        RstOutCount reader = new RstOutCount(appConfiguration);
        reader.run();
    }
